...
function InitAI(self)
    self:SetContextThink(
        ...
        function()
            ...
            self.abilities[1].Skill = LivingArmor
            self.abilities[1].SkillTrigger = function()
                return true
            end
            self.NextWayPoint = NextWayPoint
            self.Unstuck = Unstuck
            self:SetContextThink("ai_dragonspawn.aiThink", Dynamic_Wrap(self, "aiThink"), 0)
        end,
        0
    )
end

function LivingArmor(entity, ability)
    local Radius = 300
    local allies =
        FindUnitsInRadius(
