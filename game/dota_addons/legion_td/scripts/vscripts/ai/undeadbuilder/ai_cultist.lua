...
function Spawn( entityKeyValues )
    ...
    behaviorSystem = AICore:CreateBehaviorSystem( { BehaviorNone, BehaviorCastFleshHunger, BehaviorBlast} ) 
end
...
BehaviorCastFleshHunger = {}


function BehaviorCastFleshHunger:Evaluate()
    self.recomposeAbility = thisEntity:FindAbilityByName("flesh_hunger")
...
function BehaviorCastFleshHunger:Begin()
    self.recomposeAbility = thisEntity:FindAbilityByName("flesh_hunger")
    self.endTime = GameRules:GetGameTime() + .1
...
BehaviorCastFleshHunger.Continue = BehaviorCastFleshHunger.Begin --if we re-enter this ability, we might have a different target; might as well do a full reset
...
AICore.possibleBehaviors = { BehaviorNone, BehaviorCastFleshHunger, BehaviorBlast}
