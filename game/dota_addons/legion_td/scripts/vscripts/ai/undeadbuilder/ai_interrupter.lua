...
function Spawn( entityKeyValues )
    ...
    behaviorSystem = AICore:CreateBehaviorSystem( { BehaviorNone, BehaviorCastHellSpikes, BehaviorBlast} ) 
end
...
function BehaviorCastHellSpikes:Begin()
    self.recomposeAbility = thisEntity:FindAbilityByName("hell_spikes")
    self.endTime = GameRules:GetGameTime() + .1
...
BehaviorCastHellSpikes.Continue = BehaviorCastHellSpikes.Begin --if we re-enter this ability, we might have a different target; might as well do a full reset
...
AICore.possibleBehaviors = { BehaviorNone, BehaviorCastHellSpikes, BehaviorBlast}
