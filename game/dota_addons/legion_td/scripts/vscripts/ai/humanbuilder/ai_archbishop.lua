...
function BehaviorHeal:Evaluate()
	...
	if self.healAbility and self.healAbility:IsFullyCastable() then
		print ("archmage shadow wave is ready")
		...
		local mostHurt = 30 -- minimum hurt amount
		for _, unit in pairs(allies) do
			local healthLost = unit:GetMaxHealth() - unit:GetHealth()
			if healthLost > mostHurt and healthLost > (unit:GetMaxHealth() *.3) then --only cast if unit has lost at least 30% of its health
				target = unit
			end
		end
	end
