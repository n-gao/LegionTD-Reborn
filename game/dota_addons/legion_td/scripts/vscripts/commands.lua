...
function CommandEngine:CheckCommand(keys)
    if not CommandEngine.initialized then
        CommandEngine.init()
    end
...
function CommandEngine.waveInfo(waveNum, playerid)
    ...
    if waveNum == nil then
        print("waveInfo canceled: index nil")
        return
    end
    if #Game.rounds <= waveNum then
        print("waveInfo canceled: index out of bounds")
        return
    end
    ...
    if damageMin then
        damageMin = (math.floor(damageMin * 1000)) / 1000
    end -- rounding floating point from KV's
    if damageMax then
        damageMax = (math.floor(damageMax * 1000)) / 1000
    end -- rounding floating point from KV's
    if attackRate then
        attackRate = (math.floor(attackRate * 1000)) / 1000
    end -- rounding floating point from KV's
    ...
    if attackCapabilities == "DOTA_UNIT_CAP_MELEE_ATTACK" then
        attackCapabilities = "Melee"
    elseif attackCapabilities == "DOTA_UNIT_CAP_RANGED_ATTACK" then
        attackCapabilities = "Ranged"
    else
        attackCapabilities = "Can't attack"
    end
    ...
    local lines = {
        ...
        "Damage: " ..
            damageMin ..
                "-" .. damageMax .. " (" .. attackRate .. " AR) Range: " .. range .. " (" .. attackCapabilities .. ")"
    }
    ...
    for i, v in pairs(lines) do
        CustomGameEventManager:Send_ServerToPlayer(
            player,
            "waveinfo_notification",
            {text = v, duration = 15, style = {color = "black", ["font-size"] = "25px", ["line-height"] = "0px"}}
        )
    end
end
...
function CommandEngine.Commands.settings(instance, submessage, keys)
    if CommandEngine.Variables.settingsCooldown then
        return
    end
    ...
    local resettimer =
        ...
        30,
        function()
            CommandEngine.Variables.settingsCooldown = false
        end
    )
