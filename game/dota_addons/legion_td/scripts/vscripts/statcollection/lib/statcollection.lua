...
local errorDefaultModIdentifier =
    "Please change your settings.kv with a valid modID, acquired after registration of your mod on the site!"
local errorMissingSchemaIdentifier =
    "Please ensure you have a settings.kv in your statcollection folder! Missing schemaID!"
local errorDefaultSchemaIdentifier =
    "Please change your settings.kv with a valid schemaID, acquired after contacting a site admin!"
local errorInitCalledTwice =
    "Please ensure you only make a single call to statCollection:init, only the first call actually works."
...
function statCollection:calcWinnersByTeam()
    ...
    for playerID = 0, DOTA_MAX_PLAYERS do
        if PlayerResource:IsValidPlayerID(playerID) then
            output[PlayerResource:GetSteamAccountID(playerID)] =
                PlayerResource:GetTeam(playerID) == winningTeam and "1" or "0"
        end
    end
        ...
        function(keys)
            ...
            if state == DOTA_GAMERULES_STATE_CUSTOM_GAME_SETUP then
                ...
                CustomUI:DynamicHud_Create(
                    -1,
                    "statcollection",
                    "file://{resources}/layout/custom_game/statcollection.xml",
                    nil
                )
...
function statCollection:sendStage2()
    ...
    CustomGameEventManager:Send_ServerToAllClients(
        "statcollection_client",
        {modID = self.modIdentifier, matchID = self.matchID, schemaVersion = schemaVersion}
    )
