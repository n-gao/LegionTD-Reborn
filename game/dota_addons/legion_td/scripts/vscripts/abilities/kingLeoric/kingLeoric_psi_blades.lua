...
function DealDamage(keys)
	...
	local target = keys.target
	local ability = keys.ability
	local multiplier = ability:GetSpecialValueFor("dmg_multiplier")

	-- Applies the damage to the attack target
	ApplyDamage({victim = target, attacker = caster, damage = ability.first_target_damage*multiplier, damage_type = ability:GetAbilityDamageType()})
end
