...
function InitFractionKillAttribute(fraction) {
    InitAttribute("Killed " + CapitalizeFirstLetter(fraction) + " Units",
        function () {
            return GetKillsOfFraction(steam_id, fraction)
        });
}
...
(function () {
    ...
    InitAttribute("Time Played", function () {
        return ToTime(GetDataOf(steam_id, "time_played"));
    });
    InitAttribute("Level", function () {
        return GetLevelOf(steam_id);
    });
    ...
    InitAttribute("Win Rate", function () {
        return ToPercentage(GetDataOf(steam_id, "win_rate"));
    });
    InitSimpleAttribute("Won Duels", "won_duels");
    InitSimpleAttribute("Lost Duels", "lost_duels");
    InitAttribute("Duel Win Rate", function () {
        return ToPercentage(GetDataOf(steam_id, "duel_win_rate"));
    });
    InitSimpleAttribute("Earned Tangos", "earned_tangos");
    InitAttribute("Tangos per Minute", function () {
        return parseInt(GetDataOf(steam_id, "tangos_per_minute"))
    });
    InitSimpleAttribute("Earned Gold", "earned_gold");
    InitAttribute("Gold per Minute", function () {
        return parseInt(GetDataOf(steam_id, "gold_per_minute"))
    });
